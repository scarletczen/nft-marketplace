...
function Layout({ children }: Props) {
  return (
    <div className="relative w-screen h-screen bg-dark">
      <img className="absolute -top-20 left-40 h-2/4 w-1/4" src={PrimaryBlob} />
      <img className="absolute -top-20 -left-20 w-1/4" src={SecondaryBlob} />
      <img className="absolute top-0 right-0 h-1/4 w-1/4" src={SecondaryBlob} />
      <img
      ...
      <img
        ...
        src={PrimaryBlob}
      />
      <div className="glass absolute z-10 h-screen w-screen">{children}</div>
    </div>
  );
}
